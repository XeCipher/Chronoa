...
export default function RecursiveCheckbox({ task, isEditMode, onUpdate, onDelete, onAdd, depth = 0, newTaskId, setNewTaskId }: Props) {
  ...
  // Trigger naming focus & auto-select temporary text
  useEffect(() => {
    if (newTaskId === task.id) {
      ...
      setTimeout(() => {
        if (inputRef.current) {
          ...
          inputRef.current.select(); // Selects "New Item" automatically
        }
      }, 50);
    }
  ...
  // Logic to determine if we should start the beautiful exit animation
  ...
  return (
    <div 
      ref={setNodeRef} 
      style={style} 
      className={`flex flex-col w-full ${isVanishingNow ? "task-vanishing-soothing" : ""}`}
    >
      <div className={`group relative flex items-center gap-3 py-[7px] px-3 rounded-xl transition-all duration-150 ${isDragging ? "bg-[#f7f5f0] dark:bg-[#2a2a2a] shadow-md ring-1 ring-[#e0ddd5] dark:ring-[#444]" : "hover:bg-[#faf9f6] dark:hover:bg-[#222]"}`}>
        ...
        <div className="flex-1 flex items-center gap-2 min-w-0">
          {isEditingText ? (
            <input ref={inputRef} value={title} onChange={(e) => setTitle(e.target.value)} onBlur={handleBlur} onKeyDown={handleKeyDown} placeholder="Name this task..." className="flex-1 bg-transparent outline-none border-b border-[#c2956e]/50 dark:border-[#b0855f]/50 text-[#3d3b33] dark:text-[#f0f0f0] text-[14px] font-[400] pb-[2px] tracking-[0.01em] placeholder:text-[#c4c0b8] dark:placeholder:text-[#555]" />
          ) : (
        ...
        <SortableContext items={task.children.map(c => c.id)} strategy={verticalListSortingStrategy}>
          <div className="ml-[34px] mt-[1px] mb-[2px] pl-4 border-l border-[#ebe8e2] dark:border-[#333] space-y-[1px]">
            {task.children.map((child) => (
              <RecursiveCheckbox key={child.id} task={child} isEditMode={isEditMode} onUpdate={onUpdate} onDelete={onDelete} onAdd={onAdd} depth={depth + 1} newTaskId={newTaskId} setNewTaskId={setNewTaskId} />
            ))}
          </div>
        </SortableContext>
      )}
