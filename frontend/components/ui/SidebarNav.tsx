...
export default function SidebarNav() {
  ...
  const { isSidebarPinned, toggleSidebarPin, isMobileMenuOpen, toggleMobileMenu } = useUiStore();
  ...
  return (
    <>
      ...
      {isMobileMenuOpen && (
        <div 
          className="md:hidden fixed inset-0 bg-black/40 dark:bg-black/60 z-40 transition-opacity duration-300" 
      ...
      <aside
        ...
        className={`absolute md:relative h-full z-50 transition-all duration-500 ease-in-out 
          ${isMobileMenuOpen ? 'translate-x-0 w-64 shadow-2xl' : '-translate-x-full md:translate-x-0'} 
          ${isExpanded ? 'md:w-64' : isHiddenMode ? 'md:w-0' : 'md:w-20'}
        `}
      >
        ...
        <div className={`absolute inset-y-0 left-0 h-full bg-[#f7f5f0] dark:bg-[#161616] border-r border-[#e0ddd5] dark:border-[#2a2a2a] flex flex-col z-40 transition-all duration-500 ease-in-out overflow-hidden ${
          (isExpanded || isMobileMenuOpen) ? 'w-64 translate-x-0 shadow-xl shadow-[#e0ddd5]/50 dark:shadow-black/50' : 
          ...
          <div className="flex items-center h-24 relative shrink-0">
            <div className={`absolute inset-0 flex items-center px-8 transition-opacity duration-300 ${(isExpanded || isMobileMenuOpen) ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
              <h2 className="text-3xl text-[#3d3b33] dark:text-[#e0e0e0]" style={{ fontFamily: 'var(--font-cormorant), serif' }}>
                Chronoa
              </h2>
            </div>
            
            <div className={`absolute inset-0 flex items-center justify-center transition-opacity duration-300 ${(isExpanded || isMobileMenuOpen) ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
              ...
              return (
                <Link
                  ...
                  className={`flex items-center h-12 rounded-xl text-sm font-medium transition-all duration-300 overflow-hidden group
                    ${(isExpanded || isMobileMenuOpen) ? "mx-6 px-4 justify-start gap-4" : "mx-4 justify-center"}
                    ${isActive 
                ...
                >
                  ...
                  <span className={`transition-all duration-300 whitespace-nowrap ${(isExpanded || isMobileMenuOpen) ? 'opacity-100 w-auto' : 'opacity-0 w-0'}`}>
                    {item.name}
                  </span>
                </Link>
              );
          ...
          <div className="mt-auto space-y-2 pb-6 pt-4 border-t border-[#e0ddd5]/50 dark:border-[#333]/50 mx-4">
            <Link
              ...
              className={`flex items-center h-12 rounded-xl text-sm font-medium transition-all duration-300 overflow-hidden
                ${(isExpanded || isMobileMenuOpen) ? "px-4 justify-start gap-4" : "justify-center"}
                ${pathname === "/settings" ? "bg-white dark:bg-[#252525] text-[#3d3b33] dark:text-[#fff] shadow-sm" : "text-[#888888] dark:text-[#a0a0a0] hover:bg-white/50 dark:hover:bg-[#2a2a2a] hover:text-[#3d3b33] dark:hover:text-[#fff]"}
              `}
            >
